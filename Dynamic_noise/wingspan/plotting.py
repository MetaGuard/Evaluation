...
def plot():
    ...
    x_axis_label = ['Population mean of the relative error (%)', 'Population mean of the absolute error (m)', 'Population mean of R²', 'Pupulation mean of average wingspan prediction']
    file_names = ['2001_wingspan_rel_error', '2001_wingspan_abs_error', '2001_wingspan_R_2', '2001_wingspan_mean_pred_noisy_actual']
    ...
    for i, color in zip(range(0, 4), sns.color_palette()):
        ...
        if i == 3:
            ...
            part_ax.plot(epsilon_list, [1.766]*len(epsilon_list), c='black')
            part_ax.legend(['Prediction on noisy data', 'Prediction on actual data'])
