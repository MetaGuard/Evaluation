...
with open("avg_statistics.txt", "a") as f:
    ...
    print("Prediction on refresh rate: " + str(correct) + "/" + str(correct+incorrect) + " (" + str(percent) + "%) within 3 Hz", file=f)
    f.close()
...
with open("avg_statistics.txt", "a") as f:
    percent = round((correct / (correct + incorrect))*100,2)
    print("Prediction on noisy refresh rate: " + str(correct) + "/" + str(correct+incorrect) + " (" + str(percent) + "%) within 3 Hz", file=f)
    f.close()